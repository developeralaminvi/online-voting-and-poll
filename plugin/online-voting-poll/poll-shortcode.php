...
// Register the shortcode for displaying the poll
function hbuz_poll_shortcode($atts)
{
    global $hbuzpoll;
    ...
    if (empty($atts['id'])) {
        return __('Poll not found.', 'hostingbuzpoll');
    }
    ...
    if (!is_array($poll_options) || count($poll_options) == 0) {
        return __('No poll options found.', 'hostingbuzpoll');
    }
            ...
            <div class="hbuz-poll-item" id="poll-<?php echo esc_attr($poll_id); ?>"
                ...
                <div class="hbuz-poll-header">
                    <div class="hbuz-poll-date">
                        <span><i class="fa-regular fa-clock"></i> <?php echo esc_html(get_the_date('', $poll_id)); ?></span>
                    </div>
                ...
                <div class="hbuz-poll-thumbnail">
                    <?php if (has_post_thumbnail($poll_id)): ?>
                        <a href="<?php echo get_permalink($poll_id); ?>">
                            <?php echo get_the_post_thumbnail($poll_id, 'medium'); ?>
                        </a>
                    <?php endif; ?>
                </div>

                <div class="hbuz-poll-content">
                    <h2><a href="<?php echo get_permalink($poll_id); ?>"><?php echo get_the_title($poll_id); ?></a></h2>
                    <div class="hbuz-poll-excerpt">
                        <?php echo wp_trim_words(get_post_field('post_content', $poll_id), 20, '...'); ?>
                    </div>
                </div>
    ...
    <?php
    return ob_get_clean(); // Get and return the buffered output
}
add_shortcode('hostingbuz_poll', 'hbuz_poll_shortcode');
