...
<div class="container">
    <div class="poll-grid-container row">
        ...
        <?php
        // Display the current poll post first
        if ($current_poll_id):
            $poll_options = get_post_meta($current_poll_id, '_poll_options', true);
            $total_votes = get_post_meta($current_poll_id, '_total_votes', true) ?: 0;
            ?>
            <div class="col-12 col-md-4 mt-3">
                <div class="hbuz-poll-item" id="poll-<?php echo esc_attr($poll_id); ?>"
                    ...
                    <div class="hbuz-poll-header">
                        ...
                        <div class="hbuz-poll-download-btn" data-poll-id="<?php echo esc_attr($current_poll_id); ?>">
                    ...
                    <div class="hbuz-poll-options" id="poll-<?php echo esc_attr($current_poll_id); ?>">
                        <?php if (is_array($poll_options) && count($poll_options) > 0): ?>
                            <form class="hbuz-poll-vote-form" method="POST">
                                <?php foreach ($poll_options as $index => $option): ?>
                                    <?php
                                    $poll_votes = get_post_meta($current_poll_id, '_poll_votes', true) ?: [];
                                    ...
                                    $unique_option_id = $current_poll_id . '_' . $index;
                                    ?>
        ...
        <?php
        // WP_Query to get the rest of the polls from the custom post type 'hostingbuzpoll', excluding the current poll
        $paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
        $args = array(
            'post_type' => 'hostingbuzpoll',
            'posts_per_page' => 11, // Display all polls
            'paged' => $paged,
            'post__not_in' => array($current_poll_id), // Exclude the current poll
        );
        ...
        if ($poll_query->have_posts()):
            ...
            <p><?php esc_html_e('', 'hostingbuzpoll'); ?></p>
        <?php endif; ?>
